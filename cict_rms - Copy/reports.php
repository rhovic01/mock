...
if ($reportType === 'transactions' || $reportType === 'both') {
    ...
    $transactionSql = "SELECT * FROM transactions" . $transactionWhereClause . " ORDER BY transaction_date DESC LIMIT ? OFFSET ?";
...
<body>
    ...
    doc.text('CICT INVENTORY MANAGEMENT SYSTEM', 105, 20, { align: 'center' });
    
    // Add subtitle
    doc.setFontSize(14);
    doc.setTextColor(40);
    doc.setFont('helvetica', 'normal');
    doc.text('Report Summary', 105, 28, { align: 'center' });
    
    // Add divider line
    doc.setDrawColor(200);
    doc.line(15, 32, 195, 32);
    
    // Add report details in a clean layout
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Generated on: ${new Date().toLocaleString()}`, 14, 40);
    doc.text(`Generated by: <?= htmlspecialchars($_SESSION['username']) ?>`, 105, 40, { align: 'center' });
    doc.text(`Report Type: <?= ucfirst($reportType) ?>`, 196, 40, { align: 'right' });
    
    <?php if ($reportType !== 'inventory'): ?>
    doc.text(`Date Range: <?= !empty($startDate) ? $startDate : 'Start' ?> to <?= !empty($endDate) ? $endDate : 'End' ?>`, 14, 45);
    <?php endif; ?>
    
    let yPosition = 55;
    
    <?php if (($reportType === 'inventory' || $reportType === 'both') && !empty($inventoryData)): ?>
    // Inventory table with modern styling
    doc.setFontSize(12);
    doc.setTextColor(primaryColor);
    doc.setFont('helvetica', 'bold');
    ...
    inventoryData.push([
        '<?= $item['id'] ?>',
        '<?= addslashes(htmlspecialchars($item['item_name'])) ?>',
        '<?= $item['item_quantity'] ?>',
        '<?= ucfirst($item['item_availability']) ?>'
    ]);
    <?php endforeach; ?>
    ...
    doc.setFontSize(9);
    doc.setTextColor(100);
    doc.text(`Current Page Items: <?= count($inventoryData) ?> | Total Items: <?= $inventoryTotalItems ?>`, 14, yPosition);
    yPosition += 12;
    <?php endif; ?>
    ...
    const transactionHeaders = [['ID', 'Item ID', 'Student ID', 'Student Name', 'Type', 'Date', 'Status']];
    const transactionData = [];
    
    <?php foreach ($transactionData as $transaction): ?>
    transactionData.push([
        '<?= $transaction['id'] ?>',
        '<?= $transaction['item_id'] ?? 'N/A' ?>',
        '<?= addslashes(htmlspecialchars($transaction['student_id'])) ?>',
        '<?= addslashes(htmlspecialchars($transaction['student_name'])) ?>',
        '<?= ucfirst($transaction['transaction_type'] ?? 'N/A') ?>',
    ...
    doc.setFontSize(9);
    doc.setTextColor(100);
    doc.text(`Current Page Transactions: <?= count($transactionData) ?> | Total Transactions: <?= $transactionTotalItems ?>`, 14, yPosition);
    <?php endif; ?>
    ...
    doc.setFont('helvetica', 'normal');
    doc.text('© CICT Inventory Management System', 105, 285, { align: 'center' });
    
    // Save the PDF
    doc.save('CICT_IMS_Report_<?= date('Ymd_His') ?>.pdf');
}
